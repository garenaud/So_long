...
{
	if ((move_check(game, game->player.y, game->player.x + 48) != 1))
	{
		game->player.move++;
		printf(GREEN"Number of move = %d\n"ENDC, game->player.move);
		mlx_put_image_to_window(game->mlx, game->window, 
		game->image.background.mlx_img, game->player.y, game->player.x);
		game->player.x += 48;
		mlx_put_image_to_window(game->mlx, game->window, 
		game->image.char_right.mlx_img, game->player.y, game->player.x);
	}
}
...
void	left(t_game *game)
{
	if ((move_check(game, game->player.y, game->player.x - 48) != 1))
	{
		...
		game->player.x -= 48;
		mlx_put_image_to_window(game->mlx, game->window, 
		game->image.char_left.mlx_img, game->player.y, game->player.x);
	}
}

void	up(t_game *game)
{
	if ((move_check(game, game->player.y - 48, game->player.x) != 1))
	{
		game->player.move++;
		printf(GREEN"Number of move = %d\n"ENDC, game->player.move);
		mlx_put_image_to_window(game->mlx, game->window, 
		...
		game->player.y -= 48;
		mlx_put_image_to_window(game->mlx, game->window, 
		game->image.char_up.mlx_img, game->player.y, game->player.x);
	}
}
